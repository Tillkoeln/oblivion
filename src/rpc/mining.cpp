...
UniValue GetNetworkHashPS(int lookup, int height) {
    ...
    //ppcTODO - redo this to fit oblivion
...
// oblivion: get network Gh/s estimate
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated oblivion to.\n"
...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"errors\": \"...\"            (string) DEPRECATED. Same as warnings. Only shown when obliviond is started with -deprecatedrpc=getmininginfo\n"
            "}\n"
            "\nExamples:\n"
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Oblivion is not connected!");
    ...
    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Oblivion is downloading blocks...");

    unsigned int nMiningRequiresPeers = Params().MiningRequiresPeers();
    int num_blocks_of_peers, num_nodes, nBestHeight;
    {
        LOCK(cs_main);
    ...
    LogPrint(BCLog::POS, "nBestHeight: %d, num_blocks_of_peers: %d, num_nodes: %d", nBestHeight, num_blocks_of_peers, num_nodes);
...
UniValue submitblock(const JSONRPCRequest& request)
{
    ...
    // oblivion: check block before attempting to sign it
    CValidationState state;
