...
public:
    ...
    bool AcceptBlock(const std::shared_ptr<const CBlock> &pblock, CValidationState &state, const CChainParams &chainparams, CBlockIndex **ppindex, bool fRequested, const CDiskBlockPos *dbp,
                     bool *fNewBlock, bool fCheckPoS = true);
...
private:
    ...
    CBlockIndex* AddToBlockIndex(const CBlockHeader& block, bool fSetAsProofOfstake);
...
void UpdateMempoolForReorg(DisconnectedBlockTransactions &disconnectpool, bool fAddToMempool)
{
    ...
    while (it != disconnectpool.queuedTx.get<insertion_order>().rend()) {
        ...
        if (!fAddToMempool || (*it)->IsCoinBase() || (*it)->IsCoinStake() ||
            !AcceptToMemoryPool(mempool, stateDummy, *it, nullptr /* pfMissingInputs */, true /* bypass_limits */)) {
                        ...
                        bool* pfMissingInputs, int64_t nAcceptTime, bool bypass_limits)
{
    ...
    bool res = AcceptToMemoryPoolWorker(chainparams, pool, state, tx, pfMissingInputs, nAcceptTime, bypass_limits, coins_to_uncache);
    if (!res) {
...
bool IsInitialBlockDownload()
{
    ...
    if (chainActive.Tip() == nullptr)
        return true;
    if (chainActive.Tip()->nChainTrust < nMinimumChainWork)
        return true;
...
bool OblivionContextualBlockChecks(const CBlock &block, CValidationState &state, CBlockIndex *pindex, bool fJustCheck) {
    ...
    pindex->bnStakeModifier = ComputeStakeModifier(pindex->pprev, block.IsProofOfWork() ? block.GetHash() : block.vtx[1]->vin[0].prevout.hash);
    if (fJustCheck)
        return true;
...
bool static FlushStateToDisk(const CChainParams& chainparams, CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    try {
    {
        ...
        fDoFullFlush = (mode == FLUSH_STATE_ALWAYS) || fCacheLarge || fCacheCritical || fPeriodicFlush;
    ...
    do {
        ...
        while (pindexTest && !chainActive.Contains(pindexTest)) {
            ...
            if (fFailedChain || fMissingData) {
                ...
                if (fFailedChain && (pindexBestInvalid == nullptr || pindexNew->nChainTrust > pindexBestInvalid->nChainTrust))
                    pindexBestInvalid = pindexNew;
...
bool CChainState::PreciousBlock(CValidationState &state, const CChainParams &params, CBlockIndex *pindex) {
    {
        LOCK(cs_main);
        if (pindex->nChainTrust < chainActive.Tip()->nChainTrust) {
            // Nothing to do, this block is not at the tip.
            return true;
        }
        if (chainActive.Tip()->nChainTrust > nLastPreciousChainwork) {
            // The chain has been extended since the last call, reset the counter.
            nBlockReverseSequenceId = -1;
        }
        nLastPreciousChainwork = chainActive.Tip()->nChainTrust;
        setBlockIndexCandidates.erase(pindex);
...
{
    LOCK(cs_main);
    return pindexPrev ? true : false; // pindexPrev == null on genesis block
}
...
bool CChainState::AcceptBlockHeader(const CBlockHeader& block, bool fProofOfStake, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fOldClient)
{
    AssertLockHeld(cs_main);
    // Check for duplicate
    uint256 hash = block.GetHash();
...
bool ProcessNewBlockHeaders(int32_t& nPoSTemperature, const uint256& lastAcceptedHeader, const std::vector<CBlockHeader>& headers, bool fOldClient, CValidationState& state, const CChainParams& chainparams, const CBlockIndex** ppindex, CBlockHeader *first_invalid)
{
...
bool TestBlockValidity(CValidationState &state, const CChainParams &chainparams, const CBlock &block, CBlockIndex *pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot) {
    ...
    if (!ContextualCheckBlockHeader(block, block.IsProofOfStake(), state, chainparams, pindexPrev, GetAdjustedTime()))
    ...
    try {
        ...
        while (num--) {
            ...
            if (nTime + nExpiryTimeout > nNow) {
                LOCK(cs_main);
                AcceptToMemoryPoolWithTime(chainparams, mempool, state, tx, nullptr /* pfMissingInputs */, nTime, false /* bypass_limits */);
                if (state.IsValid()) {
                    ++count;
                } else {
...
bool GetCoinAge(const CTransaction &tx, const CCoinsViewCache &view, const CBlockIndex *pindexPrev, uint64_t &nCoinAge, int nDepth) {
    ...
    for (const auto &txin : tx.vin) {
        ...
        if (pblocktree->ReadTxIndex(prevout.hash, postx)) {
            ...
            if (nDepth < 0) {
                if (!CheckAge(pindexPrev, header.GetHash(), nDepth)) {
                    LogPrintf("coinage: coin age skip nDepth=%d\n", nDepth + 1);
